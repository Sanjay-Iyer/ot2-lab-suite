...
| `scripts/run_paper_print_motion_test.py`, `src/protocols/paper_print_motion_test.py`, `labware/paper_print_96_flat.json` | Self-contained experimental dry-motion island with no registry, tests, agent-tool entry, or external documentation. | It may still be a manually invoked physical-clearance aid for the paper fixture; that role cannot be disproved from repository references alone. | Confirm whether the physical paper fixture still uses this flat-labware definition; archive the trio together only if retired. |
| `configs/labware/printed_6vial_mold_20ml.yaml`, `labware/printed_6vial_mold_20ml.json` | The YAML says its dimensions are only a plausible starting point, and no protocol loads the generated JSON. | All labware YAML/JSON files are dynamically discoverable through `src/agents/labware_tools.py`; the pair may be retained as a generator example. | Decide whether it is a useful template. If not, archive the pair together under a labware-examples category. |
...
- **Confirmed unused:** the former root `agent_protocol_config.json`; archived as
  `archive/legacy_configs/agent_protocol_config.json`.
