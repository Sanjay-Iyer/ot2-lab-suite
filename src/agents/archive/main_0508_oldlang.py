...
class MockToolCallingLLM(BaseChatModel):
    """
    A mock LLM that mimics tool-calling behavior so the full LangChain
    AgentExecutor pipeline can be tested locally with zero API keys.
...
def create_opentrons_agent(use_mock: bool = False):
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", "You are a laboratory automation assistant for the Opentrons OT-2. "
                   "You can modify experimental configs, run simulations, and deploy to the physical robot. "
                   "Always simulate before deploying. "
                   "When asked to run a simulation, use the simulate_experiment tool with the config path provided."),
        ("human", "{input}"),
        MessagesPlaceholder(variable_name="agent_scratchpad"),
    ])

    # Build the agent using the generic tool-calling interface
