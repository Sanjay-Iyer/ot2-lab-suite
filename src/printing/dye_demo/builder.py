"""Build the frozen dye-dilution demo executor.
...
BASE_PROTOCOL = (
    Path(REPO_ROOT) / "src" / "protocols" / "printing" / "03_dye_dilution_print_demo.py"
)
LABWARE_DIR = Path(REPO_ROOT) / "labware"
ARTIFACT_DIR = Path(REPO_ROOT) / ".test_tmp" / "dye-demo-artifacts"
GENERATED_PATH = (
    Path(REPO_ROOT) / "src" / "protocols" / "generated" / "03_dye_dilution_print_demo_latest.py"
)
...
def render_protocol_source(
    ...
    try:
        ...
        raise DyeDemoBuildError(
            "the frozen dye-demo executor no longer contains its config sentinels"
        ) from exc
    if end < start:
        raise DyeDemoBuildError("executor config sentinels are out of order")
...
def build_dye_demo_protocol(
    config: dict[str, Any],
    *,
    run_modes: dict[str, Any] | None = None,
    output_dir: Path | str | None = None,
) -> BuiltDyeDemoProtocol:
    ...
    label = str(config.get("protocol_label", "dye_demo"))
    path = directory / f"03_dye_dilution_print_demo_{label}_{digest[:12]}.py"
    path.write_bytes(data)
    return BuiltDyeDemoProtocol(
        protocol_path=path,
...
def simulate_dye_demo_protocol(
    ...
    *,
    ...
    if not path.is_file():
        raise DyeDemoBuildError(f"protocol artifact not found: {path}")
