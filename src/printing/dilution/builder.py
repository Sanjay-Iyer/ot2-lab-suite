"""Build the frozen general-dilution executor.
...
BASE_PROTOCOL = (
    Path(REPO_ROOT) / "src" / "protocols" / "printing" / "04_general_dilution.py"
)
LABWARE_DIR = Path(REPO_ROOT) / "labware"
ARTIFACT_DIR = Path(REPO_ROOT) / ".test_tmp" / "dilution-artifacts"
GENERATED_PATH = (
    Path(REPO_ROOT) / "src" / "protocols" / "generated" / "04_general_dilution_latest.py"
)
...
class DilutionBuildError(ValueError):
...
def render_protocol_source(
    ...
    try:
        ...
        raise DilutionBuildError(
            "the frozen dilution executor no longer contains its config sentinels"
        ) from exc
    if end < start:
        raise DilutionBuildError("executor config sentinels are out of order")
...
def build_dilution_protocol(
    ...
    *,
    ...
    label = str(config.get("protocol_label", "general_dilution"))
    path = directory / f"04_general_dilution_{label}_{digest[:12]}.py"
...
def simulate_dilution_protocol(
    ...
    *,
    ...
    if not path.is_file():
        raise DilutionBuildError(f"protocol artifact not found: {path}")
