"""Build and locally simulate the frozen print-from-vial executor.

Mirrors src/printing/clover/builder.py exactly: the executor protocol is never
rewritten per experiment. Building means copying the frozen file and replacing
exactly one delimited block with the resolved configuration, so the code that
reaches the robot is byte-identical to the code that was physically validated.
...
BASE_PROTOCOL = (
    Path(REPO_ROOT) / "src" / "protocols" / "printing" / "01_print_from_vial.py"
)
LABWARE_DIR = Path(REPO_ROOT) / "labware"
ARTIFACT_DIR = Path(REPO_ROOT) / ".test_tmp" / "print-from-vial-artifacts"
GENERATED_PATH = (
    Path(REPO_ROOT) / "src" / "protocols" / "generated" / "01_print_from_vial_latest.py"
)
...
class PrintFromVialBuildError(ValueError):
    """The frozen executor could not be specialized with this configuration."""


@dataclass(frozen=True)
class BuiltPrintFromVialProtocol:
    protocol_path: Path
    protocol_sha256: str
...
def render_protocol_source(
    ...
    try:
        ...
        raise PrintFromVialBuildError(
            "the frozen print-from-vial executor no longer contains its config sentinels"
        ) from exc
    if end < start:
        raise PrintFromVialBuildError("executor config sentinels are out of order")
...
def build_print_from_vial_protocol(
    config: dict[str, Any],
    *,
    run_modes: dict[str, Any] | None = None,
    output_dir: Path | str | None = None,
) -> BuiltPrintFromVialProtocol:
    ...
    label = str(config.get("protocol_label", "print_from_vial"))
    path = directory / f"01_print_from_vial_{label}_{digest[:12]}.py"
    path.write_bytes(data)
    return BuiltPrintFromVialProtocol(
        protocol_path=path,
...
def simulate_print_from_vial_protocol(
    ...
    *,
    ...
    if not path.is_file():
        raise PrintFromVialBuildError(f"protocol artifact not found: {path}")
