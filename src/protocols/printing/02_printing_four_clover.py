"""Experiment 02 - four-clover paper printing (deterministic manual executor).

MANUAL FALLBACK WORKFLOW 2. This file needs no agent, no LLM, no runtime skill,
no natural-language request and no approval workflow. It executes exactly the
CONFIG block below and nothing else.

    configs/experiments/02_printing_four_clover.yaml   <- edit this
         |  validation + coordinate resolution
         v
    this file (CONFIG block regenerated in place)
         |
         v
    src/protocols/generated/02_printing_four_clover_latest.py  <- upload this

    python scripts/run_printing_workflow.py four-clover

The coordinate/pattern engine below is copied verbatim from the physically
validated src/protocols/printing/12_four_clover_paper_print.py (builder version
18, configs/printing/four_clover_spacing_v13.yaml). It is frozen here on purpose:
tomorrow's run must not change because an upstream file changed.
tests/test_printing_four_clover_executor.py asserts the two engines stay
byte-identical.

VALIDATED PAPER GEOMETRY. The paper surface sits at well z = 6.0 mm and
printing.dispense_height_mm is 0.5 mm above it, so droplets are released from an
absolute 6.5 mm with a 0.5 mm standoff. 0.5 mm is the height physically confirmed
to detach the droplet; do not change it without revalidating on the instrument.
...
metadata = {
    "protocolName": "Experiment 02 - Four Clover Paper Print (P20, API 2.15)",
    "author": "OT-2 Lab Suite",
    "description": (
        "Config-driven four-droplet clover printing for coffee-ring overlap "
        "experiments. Every droplet coordinate comes from "
        "configs/experiments/02_printing_four_clover.yaml. No dilution, no agent."
    ),
}
...
CONFIG = { 'protocol_version': 18,
  ...
  'destination': { 'default_clover_geometry': {'half_width_mm': 2.0, 'half_height_mm': 2.0},
                   ...
                   'manual_clover_centers': [ { 'name': 'hw2mm',
                                                'reference_well': 'F4',
                                                'geometry': { 'half_width_mm': 2.0,
                                                              'half_height_mm': 2.0}},
                                              { 'name': 'hw3mm',
                                                'reference_well': 'F7',
                                                'geometry': { 'half_width_mm': 3.0,
                                                              'half_height_mm': 3.0}},
                                              { 'name': 'hw4mm',
                                                'reference_well': 'F10',
                                                'geometry': { 'half_width_mm': 4.0,
                                                              'half_height_mm': 4.0}}],
...
def _report_drop_sequence(protocol, resolved):
    """Explain the per-drop piston contents and exit order before any motion."""
    pr = CONFIG["printing"]
    volume = float(pr["droplet_volume_ul"])
    air_gap = float(pr.get("air_gap_ul", 0.0) or 0.0)
    push_out = float(pr.get("push_out_ul", 0.0) or 0.0)
    ...
    for chase in sorted({clover["pre_air_chase_ul"] for clover in clovers}):
        ...
        protocol.comment(f"Trailing air gap:   {air_gap:.1f} uL")
        protocol.comment(f"Total piston load:  {load['total']:.1f} uL")
        if chase > 0:
            protocol.comment(
                f"1. aspirate {chase:.1f} uL AIR above the source "
                "(pre-air chase)."
            )
            protocol.comment(
                f"2. Aspirate {volume:.1f} uL LIQUID from the source."
            )
            if air_gap > 0:
                protocol.comment(
                    f"3. Aspirate {air_gap:.1f} uL trailing gap AIR for "
                    "anti-drip in transit."
                )
            protocol.comment(
                "Tip contents, piston to opening: "
                f"[{chase:.1f} uL chase air]"
                f"[{volume:.1f} uL liquid]"
                f"[{air_gap:.1f} uL gap air]"
            )
            protocol.comment(
                "After the anti-drip gap exits, liquid exits first and the chase "
                "air follows it out."
            )
            protocol.comment(
                f"Dispense {load['total']:.1f} uL piston volume; "
            ...
            protocol.comment(
                f"Aspirate {volume:.1f} uL LIQUID, then dispense "
                f"{load['total']:.1f} uL piston volume; "
                f"Liquid deposited per spot: {volume:g} uL."
            )
    protocol.comment(
    ...
    protocol.comment(f"Push-out in dispense command: {push_out:g} uL.")
    if blow_out:
        protocol.comment(
            "blow_out is enabled: a SEPARATE forceful air pulse follows the "
            "metered dispense."
        )
    else:
        protocol.comment("blow_out is disabled: no separate forceful air pulse.")
