...
CONFIG = {
    ...
    # role: "solvent" = diluent, "sample" = the material being diluted. Names are free.
    "materials": {
    ...
    # Mix the source well right before each print aspiration. reps=2 -> two up-down
    # cycles (up-down = 1 mix, up-down = 2 mixes).
    ...
    "print": {
        ...
        # Width of the paper grid. Paper columns are assigned automatically, left to
        # right, in group order; a plan needing more columns than this prints what
        # fits and reports "UNABLE TO FINISH PRINT JOB".
        "paper_columns": 12,
        # Each group is one print volume. `replicates` = how many side-by-side paper
        # columns get this volume. `droplets_per_spot` = how many drops are stacked on
        # each spot (same location, N aspirate/dispense cycles -> N x volume there).
        # Optional `paper_column: N` pins a group to a specific column; groups without
        # it fill the lowest free columns automatically.
        "groups": [
