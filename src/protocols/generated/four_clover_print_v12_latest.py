...
def _resolve_clovers(well_xy):
    ...
    default_layers = CONFIG.get("printing", {}).get("layers", 1)
...
def _preflight(protocol, labware, p20):
    ...
    if air_gap < 0 or volume + max(air_gap, 0.0) > p20_max:
        errors.append("droplet volume + nonnegative air gap must fit in the P20")
...
def _print_clovers(protocol, labware, p20, resolved):
    ...
    for clover, layer, key in resolved["plan"]:
        ...
        if push_out > 0:
            p20.dispense(volume + air_gap, destination, push_out=push_out)
        else:
            p20.dispense(volume + air_gap, destination)
        if blow_out:
