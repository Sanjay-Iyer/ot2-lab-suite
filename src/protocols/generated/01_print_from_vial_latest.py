...
CONFIG = { 'protocol_label': 'print_from_vial_test',
  'deck': { 'source': { 'slot': 7,
...
def _report_plan(protocol, resolved):
    ...
    protocol.comment("=== PRINT FROM VIAL PLAN ===")
    protocol.comment(
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    label = CONFIG.get("protocol_label", "print_from_vial")
    protocol.comment(f"=== Print From Vial {label} Started ===")
    ...
    if DEFAULT_DRY_RUN:
        ...
        protocol.comment(f"=== Print From Vial {label} Completed (dry run) ===")
        return
    _set_flow_rates(p20)
    if DEFAULT_DO_PRINT:
        _print_from_vial(protocol, labware, p20, resolved)
    protocol.comment(f"=== Print From Vial {label} Completed ===")
