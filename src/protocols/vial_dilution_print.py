...
CONFIG = {
    ...
    "deck": {
        "tuberack": {"slot": 1, "load_name": "tuberack_3dprint_20ml_8vials_v2",
                     "namespace": "custom_beta", "version": 1},
        "plate":    {"slot": 2, "load_name": "corning_96_wellplate_360ul_custom",
                     "namespace": "custom_beta", "version": 1},
        "paper":    {"slot": 3, "load_name": "corning_96_wellplate_360ul_custom",
                     "namespace": "custom_beta", "version": 1},
    ...
    "dilution": {
        "enabled": True,
        "destination_column": "1",     # which plate column holds the series (A..H of it)
        "total_volume_ul": 200.0,      # volume in every well (<= 360 well, <= 300 tip)
        # Fold factors. mode = explicit | geometric | linear | log
        #   explicit : use the `explicit` list verbatim (length = #wells)
        #   geometric: start * step_factor**i  (e.g. 2x each step -> 1,2,4,8,...)
        #   linear   : evenly spaced start..end over `count`
        #   log      : log-spaced start..end over `count`
        "factors": {
            "mode": "explicit",
        ...
        "mix_reps": 3,
        "mix_volume_ul": 120.0,
        # Single-channel dilution tips are pulled from these tiprack COLUMNS (the
        # print block column is reserved automatically). Tips auto-allocate.
    ...
    "printing": {
        "enabled": True,
        "source_column": "1",          # plate column to print (8 wells, one per channel)
        "droplet_volume_ul": 15.0,
        "num_replicates": 1,           # print the column this many times on the paper
        "paper_start_well": "A1",      # where droplet row lands on the paper reference
        "dispense_z_mm": 3.0,          # height above the paper-reference well bottom
        "replicate_spacing_mm": {"x": 12.0, "y": 0.0},  # offset between replicate columns
    ...
    "camera": {
        ...
        "capture_before": True,
        "capture_after": True,
        "capture_mid_wells": ["C1", "E1", "H1"],   # snapshot after these dilution wells
    ...
    # The robot protocol ignores this block; it documents what the CV check expects.
    "cv": {
