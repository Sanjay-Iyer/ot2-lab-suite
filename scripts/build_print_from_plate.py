#!/usr/bin/env python3
"""Build + locally simulate the print-from-plate workflow. No robot contact.

    python scripts/build_print_from_plate.py
    python scripts/build_print_from_plate.py --config configs/experiments/01b_print_from_plate.yaml
    python scripts/build_print_from_plate.py --summary

Reads configs/experiments/01b_print_from_plate.yaml (or --config), flattens it
against its referenced machine profile, writes the upload-ready artifact to
src/protocols/generated/01b_print_from_plate_latest.py, and simulates it
locally via opentrons.simulate. Nothing here contacts, discovers, or moves a
robot.
"""
...
DEFAULT_CONFIG = "configs/experiments/01b_print_from_plate.yaml"
UPLOAD_PATH = REPO / "src" / "protocols" / "generated" / "01b_print_from_plate_latest.py"
...
def main(argv: list[str] | None = None) -> int:
    ...
    from src.printing.print_from_plate.builder import (
        build_print_from_plate_protocol,
        simulate_print_from_plate_protocol,
    )
    from src.printing.print_from_plate.loader import (
        PrintFromPlateLoadError,
        load_print_from_plate_config,
    )
    ...
    try:
        config, run_modes = load_print_from_plate_config(args.config)
    except PrintFromPlateLoadError as exc:
    ...
    built = build_print_from_plate_protocol(config, run_modes=run_modes)
    ...
    passed, output = simulate_print_from_plate_protocol(
